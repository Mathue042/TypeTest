            ...
            test.action(askCallData, (ctx) => {
                if ((ctx.match == ('ans300' || 'ans301' || 'ans302'))|| bigCount == 31) {
                    ...
                    switch (true) {
                        ...
                        break
                        case (counterPt2 >32 && counterPt2 < 48): bot.telegram.sendMessage(ctx.chat.id, 'Вы Медведь. Сосите лапу')
                        break
                        case (counterPt2 > 47 && counterPt2 < 62): bot.telegram.sendMessage(ctx.chat.id, 'Вы волк. АУууууууф')
                    }
                    ctx.scene.leave()
                } else {
